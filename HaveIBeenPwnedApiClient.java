...
public class HaveIBeenPwnedApiClient {
  ...
  public static void main(String[] args) {
    ...
    HaveIBeenPwnedApiClient client = new HaveIBeenPwnedApiClient("abcd1234");

    //count for while loop and boot message if limit reached
    int count = 0;

    //while loop to prompt user to enter a password
    //the password the user enters is checked by the haveibeenpwned compromised password database
    //if the user entered password is in that database, it will make them enter a new password
    //if the user enters a compromised password 5 times, on the 5th time it will boot them from the loop.
    while(count < 5){
      ...
      count++;
      if(count == 5){
        System.out.println("You have exceed the maximum number of attempts. Please consult with I.T. to create your password. ");
        break;
      }
      boolean isPasswordPwned = client.isPasswordPwned(userPassword);
      if(isPasswordPwned){
        System.out.println("Your Password has been previously compromised, please enter a more secure password.");
      }
      else{
    ...
    // String password="d6q45s37bmyxn2bpq8";
    //System.out.println("Password "+password+" pwned = " + isPasswordPwned);

    // get breaches for an account (needs API key)
    List<Breach> breachesByAccount = client.getBreachesForAccount("email@email.com");   // foo.bar@example.com
//    System.out.println(Arrays.toString(breachesByAccount.toArray()));

    // get all breaches
    List<Breach> allBreaches = client.getBreaches();
//    System.out.println(Arrays.toString(allBreaches.toArray()));

    // find breach by name
    Breach singleBreach = client.getSingleBreach("Evite");   // ExampleBreach
    System.out.println("");
    System.out.println("");
//    System.out.println(singleBreach);

    // get pastes for an account (needs API key)
    List<Paste> pastes = client.getPastesForAccount("email@email.com");   // foo.bar@example.com
    System.out.println("");
    System.out.println("");
    System.out.println(Arrays.toString(pastes.toArray()));
